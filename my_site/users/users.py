...
import json
from my_site.validation.validator import validate, validate_update
from my_site.auth.auth import is_login
from my_site.CRUD.crud_utils import (
    save,
    get_column,
    get_user,
    to_string_table,
...
users = Blueprint(
    "users", __name__, template_folder="templates/users", static_folder="static"
)
    ...
    if is_login():
        ...
        if term:
            ...
            return render_template(
                "users.html", users=filtered_users, messages=messages, search=term
            )
